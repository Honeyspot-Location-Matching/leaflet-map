...
export class LeafletMap extends BaseElement {
  ...
  addGeoJsonToMap(data, options, addToMap = true) {
    ...
    geoJsonData.eachLayer((layer) => {
      ...
      if (options.type === "locations") {
        layer.on("click", (evt) =>
          this._handleMarkerClick(evt, layer, !addToMap, options, layer)
        );
      } else {
      ...
      if (
        ...
        const layerMarker = marker(
          [
            parseFloat(layer.feature.properties["latitude"]),
            parseFloat(layer.feature.properties["longitude"]),
          ],
          {
        ...
        layerMarker.on("click", (evt) =>
          this._handleMarkerClick(evt, layerMarker, !addToMap, options, layer)
        );
  ...
  fitMapBoundsToLayer(layerName, maxZoom = 14) {
    this.map.fitBounds(
      this.layers[layerName].items.map((item) => item._latlng || item._latlngs),
      { maxZoom: maxZoom }
    );
  }
  ...
  _handlePolygonClick(evt) {
    const properties = {
      ...evt.target.properties,
      ...evt.target.feature.properties,
    };
    if (properties.unselectable) return;
    this.dispatchEvent(
      new CustomEvent("marker-clicked", { detail: { properties: properties } })
    );
    this._deselectPolygon();
    this.selectedPolygon = evt.target;
    if (this.selectedPolygon.setStyle)
      this.selectedPolygon.setStyle({ fillColor: "#0000FF" });
  }

  _handleMarkerClick(evt, marker, removeFromMap = false, options, layer) {
    this.dispatchEvent(
      new CustomEvent("marker-clicked", {
        detail: { properties: evt.target.properties },
      })
    );
    ...
    if (!layer) {
      ...
      let markerCoordinates = [marker._latlng.lng, marker._latlng.lat];
      this.clickmark = circleMarker(
        [parseFloat(markerCoordinates[1]), parseFloat(markerCoordinates[0])],
        {
          radius: 10,
  ...
  _deselectPolygon(removeFromMap = false) {
    ...
    if (
      ...
      (this.selectedPolygon.properties &&
        this.selectedPolygon.properties.layerType === "locations")
    )
    ...
    if (removeFromMap || this.selectedPolygon.addToMap) {
      ...
      this.selectedPolygon.setStyle({
        fillColor: this.selectedPolygon.defaultOptions.style.fillColor,
      });
    }
